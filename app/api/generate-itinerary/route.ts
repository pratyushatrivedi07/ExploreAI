...
import { z } from "zod";
import { enforceTransportModes, parseItineraryJson, SYSTEM_PROMPT, buildUserPrompt } from "@/lib/itinerary";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: "Couldn't generate itinerary — try again." }, { status: 500 });
    }
    ...
    const corrected = enforceTransportModes(parsed, data.city, data.travelStyle);
