...
import { StopCard } from "@/app/trip/page";
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        setMeta({ city: data.city, days: data.days, budget: data.budget, travelStyle: data.travel_style });
        setLoading(false);
      });
  }, [slug]);
  ...
  if (notFound || !itinerary || !day) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        <Link href="/" className="text-primary underline">Plan your own trip</Link>
      </div>
    );
  }
    ...
    <div className="flex h-screen flex-col md:flex-row">
      <aside className="flex w-full flex-col border-r bg-[var(--sidebar-bg)] text-[var(--sidebar-fg)] md:w-[40%] md:max-w-[520px]">
        ...
        <div className="px-5 py-4">
          ...
          <p className="text-xs text-white/60">{meta?.days} days · {meta?.budget} · {meta?.travelStyle}</p>
        </div>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          {day.theme && <div className="text-xs uppercase tracking-wide text-white/50">{day.theme}</div>}
        ...
        <div className="border-t border-white/10 bg-black/20 p-4 text-sm">
          ...
          <div className="mt-2">
            <Link href="/" className="block w-full rounded-xl bg-primary px-4 py-2.5 text-center text-sm font-medium text-primary-foreground">
              Plan my own trip →
            </Link>
          </div>
        </div>
      ...
      <div className="relative h-[50vh] flex-1 md:h-auto">
        <JourneyGenie stops={day.stops} activeIndex={activeStop} onSelect={setActiveStop} city={meta?.city} />
      </div>
    </div>
  );
}
